...
def _sum_metrics(metrics: Sequence[EntityMetrics]) -> EntityMetrics:
    return sum(metrics, EntityMetrics())


def _evaluate_document(predictions: Sequence[PIIItem], ground_truth: Sequence[PIIItem]) -> EntityMetrics:
...
def _pii_counts(
    ...
    *,
    ...
    return matched + EntityMetrics(false_positive=unmatched_prediction, false_negative=unmatched_ground)
...
def _person_pii_counts(prediction: PIIItem, ground_truth: PIIItem) -> EntityMetrics:
    counts = EntityMetrics()
...
def _value_counts(predictions: Sequence[str], *, ground_truth: Sequence[str]) -> EntityMetrics:
