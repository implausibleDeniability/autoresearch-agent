...
1. Use a run path supplied by the user. Otherwise use the current directory when it contains `workspace/results.tsv`.
2. If the current directory has no results, inspect `git worktree list` and find autoresearch worktrees containing `workspace/results.tsv`. Use the only match. If several match, list their paths and ask the user to choose; never guess. Accept a root-level `results.tsv` only for a legacy run.
3. Run the generator from the selected worktree:
   ...
   The zero-argument command reads `workspace/results.tsv`, uses adjacent `run.log` when present, and writes `workspace/research-trajectory.svg`. Use `--results`, `--run-log`, `--no-run-log`, `--output`, or `--title` when the user specifies an override. Use `--help` for the full interface.
