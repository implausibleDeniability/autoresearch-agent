...
1. If the current `autoresearch/*` worktree contains `workspace/results.tsv`, use it. Otherwise, locate the active worktrees and use the one named by the user or whose `workspace/results.tsv` was modified most recently. For a legacy run without that path, use a root-level `results.tsv` and treat the worktree root as its artifact directory.
2. Read that worktree's `program.md` for the run and cumulative budget limits. From the artifact directory, read `HYPOTHESIS_PORTFOLIO.md` for current rankings and status and `research.md` for chronological evidence and conclusions. If the portfolio file is absent in a legacy run, use the portfolio recorded in `research.md`. Read `baseline-results.tsv` from the repository when it contains data rows. From the artifact directory's `run.log`, extract only lines matching `^(result_status|evaluation_mode|evaluation_seed|cost_is_comparable|cache_hits|cache_misses|openai_live_requests|f_score|precision|recall|api_cost_usd|duration_seconds)=`.
...
10. Build tested-hypothesis groups from result rows on the comparison dataset in first-seen order, including valid `keep` and `discard` rows. Use stable hypothesis IDs from `research.md` and `HYPOTHESIS_PORTFOLIO.md` when available. Use the arithmetic mean for repeated valid rows from the same commit and matched seed panel. Combine different commits only when the research record clearly identifies them as retries of the same hypothesis. Preserve crashes as outcomes, but never treat their zero metrics as measured performance. Use `description` for experiment intent and `finding` for the evidence-backed conclusion when present; fall back to the journal, then inspect the relevant Git log or diff only when those are unclear.
...
13. Select two to five decision-relevant summary bullets. Prioritize the strongest success, meaningful trade-offs, failures, and uncertain results instead of listing every run. Add at most one **Next** bullet for the highest-ranked explicitly documented untested hypothesis in the current portfolio. Omit categories without evidence and never invent a hypothesis.
