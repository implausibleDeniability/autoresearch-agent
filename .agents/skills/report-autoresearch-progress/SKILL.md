...
3. Accept the legacy nine-column results header and the current header with `finding` appended. Count every data row in `results.tsv` as a completed run, including crashes, debug checks, and reruns.
